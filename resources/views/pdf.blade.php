<html>

<head>
  <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
  <meta name=Generator content="Microsoft Word 15 (filtered)">
  <style>



  /* Font Definitions */
  @font-face
  {font-family:"Cambria Math";
  panose-1:2 4 5 3 5 4 6 3 2 4;}
  @font-face
  {font-family:Calibri;
    panose-1:2 15 5 2 2 2 4 3 2 4;}
    @font-face
    {font-family:"Gill Sans MT";
    panose-1:2 11 5 2 2 1 4 2 2 3;}
    /* Style Definitions */

    .page-break {
      page-break-after: always;
    }
    .s-req{

      font-size:8.0pt;
      line-height:115%;
      font-family:"Gill Sans MT",sans-serif;
    }

    .green{
      background:#92D050;


    }

    .gray{
      background:#E0E0E0;

    }


    .exterior-top{
     border-top:double 1.5pt !important;

   }



   .exterior-left{
     border-left:double 1.5pt !important;

   }


   .exterior-right{
     border-right:double 1.5pt !important;

   }

   .exterior-bottom{
     border-bottom:double 1.5pt !important;;

   }


   .normal-table-td{
    border-top:solid 1.0pt;
    border-bottom:solid 1.0pt;
    border-left:solid 1.0pt;
    border-bottom:solid 1.0pt;
    border-right:solid 1.0pt;
    padding:0cm 2.85pt 0cm 1pt
  }


  .p-content{
   margin-top:3.0pt !important;
   margin-right:0cm !important;
   margin-bottom:3.0pt !important;
   margin-left:0.5cm !important;
    text-align: justify;

 }
 .p-title{

  margin-top:3.0pt !important;
  margin-right:0cm !important;
  margin-bottom:3.0pt !important;
  margin-left:0.5cm !important;
}

.p-centrar{

 text-align: center;

}
.s-content{
  font-family:"Gill Sans MT",sans-serif;
}

.s-title{
  font-size:7.0pt;
  line-height:115%;
  font-family:"Gill Sans MT",sans-serif;
  font-weight: bold; 
}



.c-row{
  margin-left:5%;
  margin-right: 5%;
  width: 90%;
  display: block;


}

.c-col-2{
  display:  inline-block;
  width: 15%;


}

.c-col-10{
  margin-left: 30px;
  display:  inline-block;
  width: 60%;



}
p.MsoNormal, li.MsoNormal, div.MsoNormal
{margin-top:0cm;
  margin-right:0cm;
  margin-bottom:10.0pt;
  margin-left:0cm;
  line-height:115%;
  font-size:9.0pt;
  font-family:"Calibri",sans-serif;}
  p.MsoHeader, li.MsoHeader, div.MsoHeader
  {mso-style-link:"Encabezado Car";
  margin:0cm;
  margin-bottom:.0001pt;
  font-size:11.0pt;
  font-family:"Calibri",sans-serif;}
  p.MsoFooter, li.MsoFooter, div.MsoFooter
  {mso-style-link:"Pie de página Car";
  margin:0cm;
  margin-bottom:.0001pt;
  font-size:11.0pt;
  font-family:"Calibri",sans-serif;}
  p.MsoListParagraph, li.MsoListParagraph, div.MsoListParagraph
  {margin-top:0cm;
    margin-right:0cm;
    margin-bottom:10.0pt;
    margin-left:36.0pt;
    line-height:115%;
    font-size:11.0pt;
    font-family:"Calibri",sans-serif;}
    p.MsoListParagraphCxSpFirst, li.MsoListParagraphCxSpFirst, div.MsoListParagraphCxSpFirst
    {margin-top:0cm;
      margin-right:0cm;
      margin-bottom:0cm;
      margin-left:36.0pt;
      margin-bottom:.0001pt;
      line-height:115%;
      font-size:11.0pt;
      font-family:"Calibri",sans-serif;}
      p.MsoListParagraphCxSpMiddle, li.MsoListParagraphCxSpMiddle, div.MsoListParagraphCxSpMiddle
      {margin-top:0cm;
        margin-right:0cm;
        margin-bottom:0cm;
        margin-left:36.0pt;
        margin-bottom:.0001pt;
        line-height:115%;
        font-size:11.0pt;
        font-family:"Calibri",sans-serif;}
        p.MsoListParagraphCxSpLast, li.MsoListParagraphCxSpLast, div.MsoListParagraphCxSpLast
        {margin-top:0cm;
          margin-right:0cm;
          margin-bottom:10.0pt;
          margin-left:36.0pt;
          line-height:115%;
          font-size:11.0pt;
          font-family:"Calibri",sans-serif;}
          p.msonormal0, li.msonormal0, div.msonormal0
          {mso-style-name:msonormal;
            margin-right:0cm;
            margin-left:0cm;
            font-size:12.0pt;
            font-family:"Times New Roman",serif;}
            span.EncabezadoCar
            {mso-style-name:"Encabezado Car";
            mso-style-link:Encabezado;}
            span.PiedepginaCar
            {mso-style-name:"Pie de página Car";
            mso-style-link:"Pie de página";}
            .MsoChpDefault
            {font-size:10.0pt;
              font-family:"Calibri",sans-serif;}
              /* Page Definitions */
              @page WordSection1
  /*{size:612.0pt 792.0pt;
    margin:70.85pt 3.0cm 70.85pt 3.0cm;}*/



    div.WordSection1
    {page:WordSection1;}

    </style>

  </head>

  <body lang=ES-MX >

    <div class="WordSection1">

      <div class="c-row">
        <div class="c-col-2">

          <img src="img/logo2.png" style="width:150px" alt="">

        </div>
        <div class="c-col-10">
          <p class="MsoNormal" align=center style='text-align:center'><b><span style='font-family:"Gill Sans MT",sans-serif'>CÉDULA DE INFORMACIÓN DE TRÁMITES<br>
            Y SERVICIOS MUNICIPALES</span></b></p>
          </div>
        <div class="c-col-2">

          <img src="img/logo.png" style="width:150px" alt="">

        </div>
        </div>





        <p class="MsoNormal" align=center style='text-align:center'><b><span
          style='font-family:"Gill Sans MT",sans-serif'>{{$tramites->dependencia->nombre}}</span></b></p>

          <div class="placeholderclass">

            <table  class="MsoNormal"Table border=1 cellspacing=0 cellpadding=0 width="100%"
  style=';border-collapse:collapse;border:none'>
            <tr>
              <td  colspan=16 class="normal-table-td green exterior-top" >
                <p class="MsoNormal p-title">
                  <span class="s-title">NOMBRE: </span></p>
                </td>

                <td  colspan=2 class="normal-table-td green exterior-top">
                  <p class="MsoNormal p-title">
                    <span class="s-title">TRÁMITE:</span>
                  </p>
                </td>


                <td  colspan=2 class="normal-table-td exterior-top">
                  <p class="MsoNormal p-content">
                    <b>
                      <span style='s-content'>
                        @if($tramites->tipo=='Trámite')
                        X
                        @endif
                      </span>
                    </b>
                  </p>
                </td>

                <td  class="normal-table-td green exterior-top">
                  <p class="MsoNormal p-title"><span class="s-title">
                    SERVICIO: </span>
                  </p>
                </td>

                <td  class="normal-table-td exterior-top exterior-right">
                  <p class="MsoNormal p-content">
                    <b>
                      <span class="s-content">
                        @if($tramites->tipo=='Servicio')
                        X
                        @endif
                      </span>
                    </b>
                  </p>
                </td>

              </tr>




              <tr >
                <td  colspan=22 class="normal-table-td exterior-left exterior-right">
                  <p class="MsoNormal p-content p-centrar" style="margin-top:5pt" >
                    <span class="s-content">
                      {{$tramites->nombre}}
                    </span>
                  </p>
                </td>
              </tr>


              <tr>
                <td  colspan=22 class="normal-table-td green exterior-left exterior-right">
                  <p class="MsoNormal p-title">
                    <span class="s-title">
                      DESCRIPCIÓN: 
                    </span>
                  </p>
                </td>
              </tr>
              <tr style='height:66.9pt'>
                <td  colspan=22 class="normal-table-td  exterior-left exterior-right" >
                                        
          <p class="MsoNormal p-content">
            <span class="s-content" >
              {{$tramites->descripcion}}
          </span>
          </p>
                  </td>
                </tr>
                <tr style='height:31.85pt'>
                  <td  class="normal-table-td green exterior-left" >
                    <p class="MsoNormal p-title" >
                      <span class="s-title">FUNDAMENTO LEGAL:</span>
                    </p>

                  </td>
                  <td  colspan=21 class="normal-table-td  exterior-right" >
                    @foreach ($tramites->fundamento as $fundamento)

                    <p class="MsoNormal p-content">
                      <span style='s-content'>
                        {{$fundamento->articulo}}
                      </span></p>

                      @endforeach

                    </td>
                  </tr>
                  <tr>
                    <td   class="normal-table-td green exterior-left" >
                      <p class="MsoNormal p-title"><span class="s-title">
                        DOCUMENTO A OBTENER:  </span>
                      </p>
                    </td>

                    <td  colspan=13 class="normal-table-td">
                      <p class="MsoNormal p-content"><span class="s-content"> 
                        {{$tramites->doc_obtener}}
                      </span></p>
                    </td>




                    <td colspan=4  class="normal-table-td green">
                      <p class="MsoNormal p-title">
                        <span class="s-title">
                          VIGENCIA: 
                        </span>
                      </p>
                    </td>

                    <td colspan=4 class="normal-table-td  exterior-right">
                      <p class="MsoNormal p-content">
                        <span class="s-content">
                          {{$tramites->vigencia}}
                        </span>
                      </p>
                    </td>

                  </tr>



                  <tr>
                    <td  class="normal-table-td green exterior-left">
                      <p class="MsoNormal p-title">
                        <span class="s-title">¿SE REALIZA EN LÍNEA?:</span>
                      </p>

                    </td>
                    <td  class="normal-table-td ">
                      <p class="MsoNormal p-content" >
                        <span class="s-title">
                          SI 
                        </span>
                      </p>

                      @if($tramites->direccion_web=='No aplica' or $tramites->direccion_web=='No aplica ' or $tramites->direccion_web==' No aplica')
                      <p class="MsoNormal p-content"><b>
                        <span  class="s-content">
                          <br>
                        </span></b></p>                                                                                  
                        @else
                        <p class="MsoNormal p-content"><b>
                          <span  class="s-content">X</span></b></p>
                          @endif


                        </td>
                        <td  class="normal-table-td ">
                          <p class="MsoNormal p-content" >
                            <span class="s-title">NO</span>
                          </p>
                        
                      @if($tramites->direccion_web=='No aplica' or $tramites->direccion_web=='No aplica ' or $tramites->direccion_web==' No aplica')
                          <p class="MsoNormal p-content">
                            <b>
                              <span  class="s-content">
                                X
                              </span>
                            </b>
                          </p>                                                                                  
                          @else
                          <p class="MsoNormal p-content">
                            <b>
                              <span  class="s-content"> 
                                <br>
                              </span>
                            </b>
                          </p>
                          @endif


                        </td>
                        <td  colspan=4 class="normal-table-td green">
                          <p class="MsoNormal p-title"><span class="s-title">
                            DIRECCIÓN WEB:
                          </span>
                        </p>
                      </td>
                      <td  colspan=15 class="normal-table-td exterior-right">
                        <p class="MsoNormal" style='margin-top:3.0pt;margin-right:0cm;margin-bottom:3.0pt;margin-left:0.5cm'>

                          <span style='font-family:"Gill Sans MT",sans-serif'>
                            {{$tramites->direccion_web}}

                          </span></p>
                        </td>
                      </tr>

                      <tr>
                        <td  colspan=4 class="normal-table-td green exterior-left">
                          <p class="MsoNormal p-title"><span class="s-title">
                            CASOS EN LOS QUE EL TRÁMITE DEBE
                            REALIZARSE:</span>
                          </p>

                        </td>
                        <td  colspan=18 class="normal-table-td  exterior-right">
                          <p class="MsoNormal p-content">
                            <span class="s-content">

                              {{$tramites->casos}}
                            </span>
                          </p>
                        </td>
                      </tr>
                      <tr>
                        <td colspan=12 class="normal-table-td green exterior-left">
                          <p class="MsoNormal p-title p-centrar">
                            <span class="s-title">
                              REQUISITOS:
                            </span>
                          </p>
                        </td>
                        <td  colspan=10 class="normal-table-td green exterior-right">
                          <p class="MsoNormal p-title p-centrar">
                           <span class="s-title">
                            FUNDAMENTO
                            JURÍDICO-ADMINISTRATIVO, <br>
                            UTILIDAD Y DESTINO DEL REQUISITO:
                          </span>
                        </p>
                      </td>
                    </tr>
                    <tr style='height:11.4pt'>
                      <td  colspan=22 class="normal-table-td gray exterior-right exterior-left">
                        <p class="MsoNormal p-title p-centrar"><b>
                          <span class="s-title">
                            PERSONAS FÍSICAS
                          </span>
                        </b>
                      </p>
                    </td>
                  </tr>
                  @forelse ($tramites->requisitosFisicas as $requisito)

                  <tr style='height:1.0pt'>
                    <td  colspan=6 class="normal-table-td  exterior-left">
                      <p class="MsoNormal p-content"><span class="s-content" >
                        {{$requisito->descripcion}}
                      </span>
                    </p>
                  </td>

                  <td colspan=3 class="normal-table-td">
                    <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
                    margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
                    style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>ORIGINAL</span>
                  </p>
                  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
                  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
                  style='font-family:"Gill Sans MT",sans-serif'>{{$requisito->original}}</span></p>
                </td>
                <td colspan=3 class="normal-table-td">
                  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
                  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
                  style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>COPIA(S)</span></p>
                  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
                  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
                  style='font-family:"Gill Sans MT",sans-serif'>{{$requisito->copia}}</span></p>
                </td>
                <td  colspan=10 class="normal-table-td  exterior-right">
                  <p class="MsoNormal p-content" >
                    <span class="s-content">
                      {{$requisito->articulo}}
                    </span>
                  </p>
                </td>
              </tr>

              @empty
              <tr style='height:1.0pt'>
                <td  colspan=6 class="normal-table-td  exterior-left">
                  <p class="MsoNormal p-content"><span class="s-content" >
                    No aplica
                  </span>
                </p>
              </td>

              <td colspan=3 class="normal-table-td">
                <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
                margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
                style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>ORIGINAL</span>
              </p>
              <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
              margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
              style='font-family:"Gill Sans MT",sans-serif'></span></p>
            </td>
            <td colspan=3 class="normal-table-td">
              <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
              margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
              style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>COPIA(S)</span></p>
              <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
              margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
              style='font-family:"Gill Sans MT",sans-serif'></span></p>
            </td>
            <td  colspan=10 class="normal-table-td  exterior-right">
             <p class="MsoNormal p-content" >
              <span class="s-content">
                No aplica
              </span>
            </p>
          </td>
        </tr>                         
        @endforelse

        <tr style='height:5.5pt'>
          <td  colspan=22 class="normal-table-td  gray exterior-right exterior-left">
            <p class="MsoNormal p-title p-centrar"><b>
              <span class="s-title">PERSONAS JURÍDICAS COLECTIVAS</span>
            </b>
          </p>
        </td>
      </tr>

      @forelse ($tramites->requisitosMorales as $requisito)
      <tr style='height:1.0pt'>
        <td  colspan=6 class="normal-table-td  exterior-left">
          <p class="MsoNormal p-content"><span class="s-content" >
            {{$requisito->descripcion}}
          </span>
        </p>
      </td>

      <td colspan=3 class="normal-table-td">
        <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
        margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
        style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>ORIGINAL</span>
      </p>
      <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
      margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
      style='font-family:"Gill Sans MT",sans-serif'>{{$requisito->original}}</span></p>
    </td>
    <td colspan=3 class="normal-table-td">
      <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
      margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
      style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>COPIA(S)</span></p>
      <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
      margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
      style='font-family:"Gill Sans MT",sans-serif'>{{$requisito->copia}}</span></p>
    </td>
    <td  colspan=10 class="normal-table-td  exterior-right">
     <p class="MsoNormal p-content" >
      <span class="s-content">
        {{$requisito->articulo}}
      </span>
    </p>
  </td>
</tr>

@empty

<tr style='height:1.0pt'>
  <td  colspan=6 class="normal-table-td  exterior-left">
    <p class="MsoNormal p-content"><span class="s-content" >
      No aplica
    </span>
  </p>
</td>

<td colspan=3 class="normal-table-td">
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>ORIGINAL</span>
</p>
<p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
style='font-family:"Gill Sans MT",sans-serif'></span></p>
</td>
<td colspan=3 class="normal-table-td">
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>COPIA(S)</span></p>
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-family:"Gill Sans MT",sans-serif'></span></p>
</td>
<td  colspan=10 class="normal-table-td  exterior-right">
  <p class="MsoNormal p-content" >
    <span class="s-content">
      No aplica
    </span>
  </p>
</td>
</tr>

@endforelse


<tr style='height:5.5pt'>
  <td  colspan=22 class="normal-table-td gray  exterior-right exterior-left">
    <p class="MsoNormal p-title p-centrar"><b>
      <span class="s-title">INSTITUCIONES PÚBLICAS</span>
    </b>
  </p>
</td>
</tr>

@forelse ($tramites->requisitosInstituciones as $requisito)
<tr style='height:1.0pt'>
  <td  colspan=6 class="normal-table-td  exterior-left">
    <p class="MsoNormal p-content"><span class="s-content" >
      {{$requisito->descripcion}}
    </span>
  </p>
</td>

<td colspan=3 class="normal-table-td">
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>ORIGINAL</span>
</p>
<p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
style='font-family:"Gill Sans MT",sans-serif'>{{$requisito->original}}</span></p>
</td>
<td colspan=3 class="normal-table-td">
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>COPIA(S)</span></p>
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-family:"Gill Sans MT",sans-serif'>{{$requisito->copia}}</span></p>
</td>
<td  colspan=10 class="normal-table-td  exterior-right">
  <p class="MsoNormal p-content" >
    <span class="s-content">
      {{$requisito->articulo}}
    </span>
  </p>
</td>
</tr>

@empty

<tr style='height:1.0pt'>
  <td  colspan=6 class="normal-table-td  exterior-left">
    <p class="MsoNormal p-content"><span class="s-content" >
      No aplica
    </span>
  </p>
</td>

<td colspan=3 class="normal-table-td">
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>ORIGINAL</span>
</p>
<p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
style='font-family:"Gill Sans MT",sans-serif'></span></p>
</td>
<td colspan=3 class="normal-table-td">
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>COPIA(S)</span></p>
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-family:"Gill Sans MT",sans-serif'></span></p>
</td>
<td  colspan=10 class="normal-table-td  exterior-right">
  <p class="MsoNormal p-content" >
    <span class="s-content">
      No aplica
    </span>
  </p>
</td>
</tr>

@endforelse

<tr style='height:5.5pt'>
  <td  colspan=22 class="normal-table-td gray exterior-right exterior-left">
    <p class="MsoNormal p-title p-centrar"><b>
      <span class="s-title">OTROS</span>
    </b>
  </p>
</td>
</tr>

<tr style='height:1.0pt'>
  <td  colspan=6 class="normal-table-td  exterior-left">
    <p class="MsoNormal p-content"><span class="s-content" >
      No aplica
    </span>
  </p>
</td>

<td colspan=3 class="normal-table-td">
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>ORIGINAL</span>
</p>
<p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
style='font-family:"Gill Sans MT",sans-serif'></span></p>
</td>
<td colspan=3 class="normal-table-td">
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-size:7.0pt;line-height:115%;font-family:"Gill Sans MT",sans-serif'>COPIA(S)</span></p>
  <p class="MsoNormal" align=center style='margin-top:4.0pt;margin-right:0cm;
  margin-bottom:4.0pt;margin-left:0cm;text-align:center'><span
  style='font-family:"Gill Sans MT",sans-serif'></span></p>
</td>
<td  colspan=10 class="normal-table-td  exterior-right">
 <p class="MsoNormal p-content">
  <span class="s-content" >
    No aplica
  </span>
</p>
</td>
</tr>
<tr>
  <td colspan=2 class="normal-table-td green exterior-left">
   <p class="MsoNormal p-title"><span class="s-title">
    DURACIÓN DEL TRÁMITE:
  </span>
</p>
</td>
<td  colspan=7 class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">
      {{$tramites->duracion}}
    </span>
  </p>
</td>
<td colspan=4 class="normal-table-td green">
 <p class="MsoNormal p-title">
  <span class="s-title">
    TIEMPO DE RESPUESTA:
  </span>
</p>
</td>
<td  colspan=9 class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">
      {{$tramites->tiempo_rep}}
    </span>
  </p>
</td>
</tr>

<tr style='height:1.0pt'>
  <td  colspan=2 class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        VIGENCIA:
      </span>
    </p>
  </td>
  <td  colspan=20 class="normal-table-td  exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">
        {{$tramites->vigencia}}
      </span>
    </p>
  </td>
</tr>
<tr style='height:1.0pt'>
  <td colspan=2 class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        COSTO:
      </span>
    </p>
  </td>
  <td  colspan=20 class="normal-table-td  exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">
        {{$tramites->costo}}
      </span>
    </p>
  </td>
</tr>
<tr style='height:1.0pt'>
  <td  colspan=2 class="normal-table-td green  exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        FORMA DE PAGO:
      </span>
    </p>
  </td>
  <td  colspan=3 class="normal-table-td gray">
    <p class="MsoNormal p-title">
      <span class="s-title">
        EFECTIVO
      </span>
    </p>
  </td>
  <td  class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
        @if($tramites->formaDePago["efectivo"]==1)
          <b>X</b>
        @else
          &nbsp;
        @endif
      </span>
    </p>
    </td>
    <td  colspan=4 class="normal-table-td gray">
      <p class="MsoNormal p-title"><span class="s-title">TARJETA DE
        CRÉDITO </span></p>
      </td>
      <td  class="normal-table-td">
        <p class="MsoNormal p-content">
          <span class="s-content">
           @if($tramites->formaDePago["credito"]==1)
              <b>X</b>
            @else
                &nbsp;
            @endif
          </span>
        </p>
      </td>
      <td  colspan=4 class="normal-table-td gray">
        <p class="MsoNormal p-title">
          <span class="s-title">
            TARJETA DE DÉBITO
          </span>
        </p>
      </td>
      <td  class="normal-table-td">
        <p class="MsoNormal p-content">
          <span class="s-content">
             @if($tramites->formaDePago["debito"]==1)
              <b>X</b>
            @else
                &nbsp;
            @endif
          </span>
        </p>
      </td>
      <td  colspan=5 class="normal-table-td gray">
        <p class="MsoNormal p-title"><span class="s-title">EN LÍNEA
          (PORTAL DE PAGOS)</span></p>
        </td>
        <td  class="normal-table-td exterior-right">
          <p class="MsoNormal p-content"><span class="s-content">
            &nbsp;
          </span>
        </p>
      </td>
    </tr>

    <tr style='height:1.0pt'>
      <td  colspan=2 class="normal-table-td green exterior-left">
        <p class="MsoNormal p-title">
          <span class="s-title">
            DÓNDE PODRÁ PAGARSE:
          </span>
        </p>
      </td>
      <td  colspan=20 class="normal-table-td exterior-right">
        <p class="MsoNormal p-content">
          <span class="s-content">
            {{$tramites->dondepag}}
          </span>
        </p>
      </td>
    </tr>
    <tr style='height:1.0pt'>
      <td  colspan=2 class="normal-table-td green exterior-left">
        <p class="MsoNormal p-title">
          <span class="s-title">
            OTRAS ALTERNATIVAS:
          </span>
        </p>
      </td>
      <td  colspan=20 class="normal-table-td exterior-right">
        <p class="MsoNormal p-content">
          <span class="s-content">
            {{$tramites->alternativa_pago}}
          </span>
        </p>
      </td>
    </tr>
    <tr  style='height:1.0pt'>
      <td colspan=2 class="normal-table-td green exterior-left">
        <p class="MsoNormal p-title">
          <span class="s-title">
            CRITERIOS DE RESOLUCION DEL TRAMITE:
          </span>
        </p>
      </td>
      <td  colspan=20 class="normal-table-td exterior-right">
        <p class="MsoNormal p-content">
          <span class="s-content">
            {{$tramites->criterios}}
          </span>
        </p>
      </td>
    </tr>
    
  </table>


@if($tramites->id!=1)
    <p class="MsoNormal">&nbsp;</p>
@endif

@if($tramites->id==2)
   <div class="page-break"></div>

@endif

  <table  class="MsoNormal"Table border=1 cellspacing=0 cellpadding=0 width="100%"
  style=';border-collapse:collapse;border:none'>

  <tr>
    <td  colspan=15 class="normal-table-td green exterior-left exterior-right  exterior-top">
      <p class="MsoNormal p-title">
        <span class="s-title p-centrar">
          DEPENDENCIA U ORGANISMO:
        </span>
      </p>
    </td>    
  </tr>

  <tr style='height:1.0pt'>
    <td  colspan=15 class="normal-table-td exterior-left exterior-right">
      <p class="MsoNormal p-content p-centrar">
        <span class="s-content">
          {{$tramites->dependencia->nombre}}
        </span>
      </p>
    </td>
  </tr>

  <tr>
    <td  colspan=4 class="normal-table-td green exterior-left">
      <p class="MsoNormal p-title">
        <span class="s-title">
          TITULAR DE LA DEPENDENCIA:
        </span>
      </p>
    </td>
    <td  colspan=11 class="normal-table-td  exterior-right">
      <p class="MsoNormal p-content">
        <span class="s-content">

         {{$tramites->oficinaDepedencia[0]->titular}}
       </span>
     </p>
   </td>
 </tr>

  <tr>
    <td  colspan=15 class="normal-table-td green exterior-left exterior-right">
      <p class="MsoNormal p-title p-centrar">
        <span class="s-title">
          UNIDAD ADMINISTRATIVA RESPONSABLE:
        </span>
      </p>
    </td>
  </tr>
  <tr>
    <td  colspan=15 class="normal-table-td  exterior-left exterior-right  ">
      <p class="MsoNormal p-content p-centrar">
        <span class="s-content">
          {{$tramites->puesto_elabora}}
        </span>
      </p>

    </td>
  </tr>
<tr>
    <td  colspan=4 class="normal-table-td green exterior-left">
      <p class="MsoNormal p-title">
        <span class="s-title">
          TITULAR DE LA UNIDAD ADMINISTRATIVA RESPONSABLE:
        </span>
      </p>
    </td>
    <td  colspan=11 class="normal-table-td  exterior-right">
      <p class="MsoNormal p-content">
        <span class="s-content">

         {{$tramites->oficinaUnidad[0]->titular}}
       </span>
     </p>
   </td>
 </tr>
  

 <tr>
  <td  class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        DOMICILIO:
      </span>
    </p>
  </td>
  <td   class="normal-table-td gray">
    <p class="MsoNormal p-title">
      <span class="s-title">CALLE:
      </span>
    </p>
  </td>
  <td  colspan=6 class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
       {{$tramites->oficinaUnidad[0]->calle}}
     </span>
   </p>
 </td>
 <td   class="normal-table-td gray">
  <p class="MsoNormal p-title">
    <span class="s-title">
      NO. INT:
    </span>
  </p>
</td>
<td  colspan=2 class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">
     {{$tramites->oficinaUnidad[0]->numint}}

   </span>
 </p>
</td>

<td  class="normal-table-td gray">
  <p class="MsoNormal p-title">
    <span class="s-title">
      NO. EXT:
    </span>
  </p>
</td>
<td   colspan=3 class="normal-table-td exterior-right">
  <p class="MsoNormal p-content">
    <span class="s-content">
     {{$tramites->oficinaUnidad[0]->numext}}

   </span>
 </p>
</td>

</tr>

<tr>
  <td   class="normal-table-td gray exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        COLONIA:
      </span>
    </p>
  </td>
  <td  colspan=8 class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
       {{$tramites->oficinaUnidad[0]->colonia}}
     </span>
   </p>
 </td>
 <td  colspan=2 class="normal-table-td gray">
  <p class="MsoNormal p-title">
    <span class="s-title">
      MUNICIPIO:
    </span>
  </p>
</td>
<td  colspan=4 class="normal-table-td exterior-right">
  <p class="MsoNormal p-content">
    <span class="s-content">
      Cuautitlán Izcalli
    </span>
  </p>
</td>
</tr>

<tr>
  <td class="normal-table-td gray exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        C.P.:
      </span>
    </p>
  </td>
  <td  colspan=3  class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
       {{$tramites->oficinaUnidad[0]->CP}}

     </span>
   </p>
 </td>

 <td colspan=5 class="normal-table-td green">
  <p class="MsoNormal p-title">
    <span class="s-title">
      HORARIO Y DÍAS DE ATENCIÓN:
    </span>
  </p>
</td>

<td colspan=6 class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">
     {{$tramites->oficinaUnidad[0]->horario}}
   </span>
 </p>
</td>
</tr>


<tr>
  <td   class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        LADA:
      </span>
    </p>
  </td>
  <td  colspan=8  class="normal-table-td green">
    <p class="MsoNormal p-title">
      <span class="s-title">
        TELÉFONOS:
      </span>
    </p>
  </td>
  <td  class="normal-table-td green">
    <p class="MsoNormal p-title">
      <span class="s-title">
        EXTS.:
      </span>
    </p>
  </td>
  <td  colspan=2 class="normal-table-td green">
    <p class="MsoNormal p-title">
      <span class="s-title">
        FAX:
      </span>
    </p>
  </td>
  <td  colspan=3 class="normal-table-td green exterior-right">
    <p class="MsoNormal p-title">
      <span class="s-title">
        CORREO ELECTRÓNICO:
      </span>
    </p>
  </td>
</tr>

<tr >
  <td  class="normal-table-td exterior-left">
    <p class="MsoNormal p-content">
      <span class="s-content">
       {{$tramites->oficinaUnidad[0]->lada}}

     </span>
   </p>
 </td>

 <td colspan=8 class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">

     {{str_replace('"','',$tramites->oficinaUnidad[0]->telefono)}}



   </span>
 </p>
</td>

<td  class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">
      {{str_replace('"','',$tramites->oficinaUnidad[0]->extension)}}

    </span>
  </p>
</td>

<td  colspan=2 class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">

      No aplica
    </span>
  </p>
</td>

<td  colspan=3 class="normal-table-td exterior-right">
  <p class="MsoNormal p-content">
    <span class="s-content">
     {{$tramites->oficinaUnidad[0]->mail}}
   </span>
 </p>
</td>

</tr>




<tr>
  <td colspan=15 class="normal-table-td gray exterior-right exterior-left">
    <p class="MsoNormal p-content p-centrar">
      <b>
        <span class="s-content">
          OTRAS OFICINAS QUE PRESTAN EL SERVICIO
        </span>
      </b>
    </p>
  </td>
</tr>


@for ($i = 0; $i < count($tramites->oficinaOtro); $i++)

@if(count($tramites->oficinaOtro)>1)

<tr>
  <td colspan=15 class="normal-table-td gray exterior-right exterior-left">
    <p class="MsoNormal p-content p-centrar">
      <b>
        <span class="s-content">
          OFICINA {{$i+1}}:
        </span>
      </b>
    </p>
  </td>
</tr>

@endif




<tr>


  <td  colspan=6 class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        OFICINA:
      </span>
    </p>
  </td>

  <td  colspan=9 class="normal-table-td  exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">
        {{$tramites->oficinaOtro[$i]->oficina}}
      </span>
    </p>
  </td>

</tr>
<tr>
  <td  colspan=4 class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        RESPONSABLE DE LA OFICINA:
      </span>
    </p>
  </td>
  <td  colspan=11 class="normal-table-td  exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">

       {{$tramites->oficinaOtro[$i]->titular}}
     </span>
   </p>
 </td>
</tr>

<tr>

  <td  class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        DOMICILIO:
      </span>
    </p>
  </td>
  <td   class="normal-table-td gray">
    <p class="MsoNormal p-title">
      <span class="s-title">CALLE:
      </span>
    </p>
  </td>
  <td  colspan=6 class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
       {{$tramites->oficinaOtro[$i]->calle}}
     </span>
   </p>
 </td>
 <td   class="normal-table-td gray">
  <p class="MsoNormal p-title">
    <span class="s-title">
      NO. INT:
    </span>
  </p>
</td>
<td  colspan=2 class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">
     {{$tramites->oficinaOtro[$i]->numint}}

   </span>
 </p>
</td>

<td  class="normal-table-td gray">
  <p class="MsoNormal p-title">
    <span class="s-title">
      NO. EXT:
    </span>
  </p>
</td>
<td   colspan=3 class="normal-table-td exterior-right">
  <p class="MsoNormal p-content">
    <span class="s-content">
     {{$tramites->oficinaOtro[$i]->numext}}

   </span>
 </p>
</td>

</tr>

<tr>
  <td   class="normal-table-td gray exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        COLONIA:
      </span>
    </p>
  </td>
  <td  colspan=8 class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
       {{$tramites->oficinaOtro[$i]->colonia}}
     </span>
   </p>
 </td>
 <td  colspan=2 class="normal-table-td gray">
  <p class="MsoNormal p-title">
    <span class="s-title">
      MUNICIPIO:
    </span>
  </p>
</td>
<td  colspan=4 class="normal-table-td exterior-right">
  <p class="MsoNormal p-content">
    <span class="s-content">
      Cuautitlán Izcalli
    </span>
  </p>
</td>
</tr>

<tr>
  <td class="normal-table-td gray exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        C.P.:
      </span>
    </p>
  </td>
  <td  colspan=3  class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
       {{$tramites->oficinaOtro[$i]->CP}}

     </span>
   </p>
 </td>

 <td colspan=5 class="normal-table-td green">
  <p class="MsoNormal p-title">
    <span class="s-title">
      HORARIO Y DÍAS DE ATENCIÓN:
    </span>
  </p>
</td>

<td colspan=6 class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">
     {{$tramites->oficinaOtro[$i]->horario}}
   </span>
 </p>
</td>
</tr>


<tr>
  <td   class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        LADA:
      </span>
    </p>
  </td>
  <td  colspan=8  class="normal-table-td green">
    <p class="MsoNormal p-title">
      <span class="s-title">
        TELÉFONOS:
      </span>
    </p>
  </td>
  <td  class="normal-table-td green">
    <p class="MsoNormal p-title">
      <span class="s-title">
        EXTS.:
      </span>
    </p>
  </td>
  <td  colspan=2 class="normal-table-td green">
    <p class="MsoNormal p-title">
      <span class="s-title">
        FAX:
      </span>
    </p>
  </td>
  <td  colspan=3 class="normal-table-td green exterior-right">
    <p class="MsoNormal p-title">
      <span class="s-title">
        CORREO ELECTRÓNICO:
      </span>
    </p>
  </td>
</tr>

<tr >
  <td  class="normal-table-td exterior-left">
    <p class="MsoNormal p-content">
      <span class="s-content">
       {{$tramites->oficinaOtro[$i]->lada}}

     </span>
   </p>
 </td>

 <td colspan=8 class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">
     {{$tramites->oficinaOtro[$i]->telefono}}
   </span>
 </p>
</td>

<td  class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">
     {{$tramites->oficinaOtro[$i]->extension}}
   </span>
 </p>
</td>

<td  colspan=2 class="normal-table-td">
  <p class="MsoNormal p-content">
    <span class="s-content">

      No aplica
    </span>
  </p>
</td>

<td  colspan=3 class="normal-table-td exterior-right">
  <p class="MsoNormal p-content">
    <span class="s-content">
     {{$tramites->oficinaOtro[$i]->mail}}
   </span>
 </p>
</td>

</tr>




<tr >
  <td  colspan=5 class="normal-table-td  green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        MUNICIPIOS QUE ATIENDE:
      </span>
    </p>
  </td>
  <td  colspan=10 class="normal-table-td  exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">
        {{$tramites->oficinaOtro[$i]->municipios_a}}

      </span>
    </p>
  </td>
</tr>




@endfor





@if(count($tramites->oficinaOtro)==0)


<tr>


  <td  colspan=6 class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        OFICINA:
      </span>
    </p>
  </td>

  <td  colspan=9 class="normal-table-td  exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica
      </span>
    </p>
  </td>

</tr>
<tr>
  <td  colspan=4 class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        RESPONSABLE DE LA OFICINA:
      </span>
    </p>
  </td>
  <td  colspan=11 class="normal-table-td  exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">

        No aplica
      </span>
    </p>
  </td>
</tr>

<tr>

  <td  class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        DOMICILIO:
      </span>
    </p>
  </td>
  <td   class="normal-table-td gray">
    <p class="MsoNormal p-title">
      <span class="s-title">CALLE:
      </span>
    </p>
  </td>
  <td  colspan=6 class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica
      </span>
    </p>
  </td>
  <td  colspan=1 class="normal-table-td gray">
    <p class="MsoNormal p-title">
      <span class="s-title">
        NO. INT:
      </span>
    </p>
  </td>
  <td  colspan=2 class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica

      </span>
    </p>
  </td>

  <td  class="normal-table-td gray">
    <p class="MsoNormal p-title">
      <span class="s-title">
        NO. EXT:
      </span>
    </p>
  </td>
  <td   colspan=3 class="normal-table-td exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica

      </span>
    </p>
  </td>

</tr>

<tr>
  <td  colspan=2 class="normal-table-td gray exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        COLONIA:
      </span>
    </p>
  </td>
  <td  colspan=4 class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica
      </span>
    </p>
  </td>
  <td  colspan=2 class="normal-table-td gray">
    <p class="MsoNormal p-title">
      <span class="s-title">
        MUNICIPIO:
      </span>
    </p>
  </td>
  <td  colspan=7 class="normal-table-td exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica
      </span>
    </p>
  </td>
</tr>

<tr>
  <td class="normal-table-td gray exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        C.P.:
      </span>
    </p>
  </td>
  <td  colspan=3  class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica

      </span>
    </p>
  </td>

  <td colspan=5 class="normal-table-td green">
    <p class="MsoNormal p-title">
      <span class="s-title">
        HORARIO Y DÍAS DE ATENCIÓN:
      </span>
    </p>
  </td>

  <td colspan=6 class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica
      </span>
    </p>
  </td>
</tr>


<tr>
  <td   class="normal-table-td green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        LADA:
      </span>
    </p>
  </td>
  <td  colspan=8  class="normal-table-td green">
    <p class="MsoNormal p-title">
      <span class="s-title">
        TELÉFONOS:
      </span>
    </p>
  </td>
  <td  class="normal-table-td green">
    <p class="MsoNormal p-title">
      <span class="s-title">
        EXTS.:
      </span>
    </p>
  </td>
  <td  colspan=2 class="normal-table-td green">
    <p class="MsoNormal p-title">
      <span class="s-title">
        FAX:
      </span>
    </p>
  </td>
  <td  colspan=3 class="normal-table-td green exterior-right">
    <p class="MsoNormal p-title">
      <span class="s-title">
        CORREO ELECTRÓNICO:
      </span>
    </p>
  </td>
</tr>

<tr >
  <td  class="normal-table-td exterior-left">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica

      </span>
    </p>
  </td>

  <td colspan=8 class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica
      </span>
    </p>
  </td>

  <td  class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica
      </span>
    </p>
  </td>

  <td  colspan=2 class="normal-table-td">
    <p class="MsoNormal p-content">
      <span class="s-content">

        No aplica
      </span>
    </p>
  </td>

  <td  colspan=3 class="normal-table-td exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica
      </span>
    </p>
  </td>

</tr>




<tr >
  <td  colspan=5 class="normal-table-td  green exterior-left">
    <p class="MsoNormal p-title">
      <span class="s-title">
        MUNICIPIOS QUE ATIENDE:
      </span>
    </p>
  </td>
  <td  colspan=10 class="normal-table-td  exterior-right">
    <p class="MsoNormal p-content">
      <span class="s-content">
        No aplica
      </span>
    </p>
  </td>
</tr>

@endif


<tr>
  <td colspan=15 class="normal-table-td gray  exterior-left exterior-right">
    <p class="MsoNormal p-content p-centrar">
      <b>
        <span class="s-content">
          PREGUNTAS FRECUENTES
        </span>
      </b>
    </p>
  </td>
</tr>


@for ($i = 0; $i < count($tramites->pregunta); $i++)
<tr>
  <td colspan=5 class="normal-table-td green  exterior-left ">
    <p class="MsoNormal p-title">
      <span class="s-title">
        PREGUNTA FRECUENTE {{ $i+1 }}:
      </span>
    </p>
  </td>
  <td  colspan=10 class="normal-table-td  exterior-right ">
    <p class="MsoNormal p-content">
      <span class="s-content">
        {{$tramites->pregunta[$i]->pregunta}}
      </span>
    </p>
  </td>
</tr>
<tr>
  <td  colspan=5 class="normal-table-td   exterior-left ">
    <p class="MsoNormal p-title">
      <span class="s-title">
        RESPUESTA:
      </span>
    </p>
  </td>
  <td  colspan=10 class="normal-table-td  exterior-right ">
    <p class="MsoNormal p-content">
      <span class="s-content">
        {{$tramites->pregunta[$i]->respuesta}}
      </span>
    </p>
  </td>
</tr>
@endfor


</table>


<p class="MsoNormal">&nbsp;</p>


@if($tramites->saltoPagina)
<div class="page-break"></div>
@endif

<table  class="MsoNormal"Table border=1 cellspacing=0 cellpadding=0 width="100%"
style=';border-collapse:collapse;border:none'>
<tr >

  <td  colspan=5 valign=top style='height:50.15pt; border-top:double windowtext 2.25pt;
  border-left:double windowtext 2.25pt;border-bottom:none;border-right:double windowtext 2.25pt;
  background:white;padding:0cm 2.85pt 0cm 2.85pt'>
  <p class="MsoNormal p-content p-centrar">
    <span style='s-content'>
      ELABORÓ:
    </span>
  </p>
</td>
<td  colspan=5 valign=top style='height:50.15pt; border-top:double windowtext 2.25pt;
border-left:double windowtext 2.25pt;border-bottom:none;border-right:double windowtext 2.25pt;
background:white;padding:0cm 2.85pt 0cm 2.85pt'>
<p class="MsoNormal p-content p-centrar">
  <span style='s-content'>
    VISTO BUENO:
  </span>
</p>
</td>
<td  colspan=5 valign=top style='height:50.15pt; border-top:double windowtext 2.25pt;
border-left:double windowtext 2.25pt;border-bottom:none;border-right:double windowtext 2.25pt;
background:white;padding:0cm 2.85pt 0cm 2.85pt'>
<p class="MsoNormal p-content p-centrar">
  <span style='s-content'>
    AUTORIZÓ:
  </span>
</p>
</td>
</tr>

<tr >

  <td  colspan=5 valign=top style='border-top:none;
  border-left:double windowtext 2.25pt;border-bottom:double windowtext 2.25pt;border-right:double windowtext 2.25pt;
  background:white;padding:0cm 2.85pt 0cm 2.85pt'>
  <p class="MsoNormal p-content p-centrar">
    <span style='s-content'>
      <u>{{$tramites->elabora}}</u>
    </span>
  </p>
  <p class="MsoNormal p-content p-centrar">
    <span style='s-content'>
       {{$tramites->tituloElabora}}
    </span>
  </p>
</td>
<td  colspan=5 valign=top style='border-top:none;
border-left:double windowtext 2.25pt;border-bottom:double windowtext 2.25pt;border-right:double windowtext 2.25pt;
background:white;padding:0cm 2.85pt 0cm 2.85pt'>
<p class="MsoNormal p-content p-centrar">
  <span style='s-content'>
   @if($tramites->revisa=='Martín Castañón Vargas (Encargado de Despacho)')
<u>Martín Castañón Vargas</u>
@else
    <u>
      {{$tramites->revisa}}
    </u>
 @endif  
  </span>
</p>
<p class="MsoNormal p-content p-centrar">
  <span style='s-content'>
@if($tramites->revisa=='Martín Castañón Vargas (Encargado de Despacho)')

  Encargado de Despacho de la Unidad Municipal de
Protección Civil y Bomberos 

@else
    {{$tramites->tituloRevisa}}
 @endif    
  </span>
</p>
</td>
<td  colspan=5 valign=top style='border-top:none;
border-left:double windowtext 2.25pt;border-bottom:double windowtext 2.25pt;border-right:double windowtext 2.25pt;
background:white;padding:0cm 2.85pt 0cm 2.85pt'>
<p class="MsoNormal p-content p-centrar">
  <span style='s-content'>

@if($tramites->autoriza== 'Martín Castañón Vargas (Encargado de Despacho)')
<u>Martín Castañón Vargas</u>
@else
   
      <u>{{$tramites->autoriza}}</u>

 @endif   
  </span>
</p>
<p class="MsoNormal p-content p-centrar">
  <span style='s-content'>
    @if($tramites->autoriza=='Martín Castañón Vargas (Encargado de Despacho)')

  Encargado de Despacho de la Unidad Municipal de
Protección Civil y Bomberos 

@else
    {{$tramites->tituloAutoriza}}
 @endif   

  </span>
</p>
</td>
</tr>


<tr >

  <td  colspan=15 class="normal-table-td exterior-top exterior-left exterior-right exterior-bottom">
    <p class="MsoNormal p-content p-centrar">
      <span style='s-content'>
        FECHA DE ACTUALIZACIÓN
      </span>
    </p>
    <p class="MsoNormal p-contetn p-centrar">
      <span style='s-content'>
        14/08/2017
      </span>
    </p>
    
  </td>


</tr>
</table>

</div>



</body>

</html>
